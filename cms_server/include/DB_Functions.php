...
class DB_Functions {
    ...
    public function search($tag_names, $types){
       $sql_query = "SELECT * FROM object INNER JOIN tag ON object.id = tag.object_id WHERE tag.tag_name IN (";
        ...
        for($j=0;$j<count($types)-1;$j++){
            $sql_query = $sql_query."'".$types[$j]."',";
        }
        $sql_query = $sql_query."'".$types[count($types)-1]."') GROUP BY object.id;";
    ...
    public function addtoPlaylist($objs, $user_id, $playlist_name){
        for($i=0;$i<count($objs);$i++){
            $stmt = $this->conn->prepare("INSERT INTO playlist (object_id, user_id, playlist_name) VALUES(?, ?, ?)");
            // $stmt->bind_param("ssss", $username, $email, $encrypted_password, $salt);
    ...
    public function getPlaylists($user_id) {
        ...
        $stmt = $this->conn->prepare("SELECT * FROM playlist INNER JOIN object ON object.id = playlist.object_id WHERE user_id=? group by object.id,playlist.playlist_name;");
